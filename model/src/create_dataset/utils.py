...
def get_note_density(mid):
    ...
    n_notes = len((for instrument in mid.instruments for note in instrument.notes))
    density = n_notes / duration
    return density
...
def get_n_instruments(mid):
    n_instruments = len((for instrument in mid.instruments if instrument.notes != []))
...
def fix_string(s):
    if s != "":
        ...
        s = re.sub("[\(\[].*?[\)\]]", "", s)    # remove everything in parentheses
